...
`;
  ...
  private async initializeCategoryFlow(state: SessionState, slug: string, name?: string) {
    // Map specific cleaning sub-categories to the main 'temizlik-hizmetleri' graph if applicable
    let graphSlug = slug;
    if (['ev-temizligi', 'ofis-temizligi', 'insaat-sonrasi-temizlik'].includes(slug)) {
