      ...
      if (this.geminiService.isAvailable()) {
        ...
        if (state.step === 'confirm_form') {
          if (message.toLowerCase().includes('onayla') || message.toLowerCase().includes('evet') || message.toLowerCase().includes('doğru')) {
            ...
            if (!seeker) {
              throw new BadRequestException('Müşteri kaydı bulunamadı. Lütfen giriş yaptığınızdan veya telefon doğrulamasını tamamladığınızdan emin olun.');
            }
            ...
            // Delegate distribution to BullMQ queue (TaleplerProcessor)
            try {
              await this.distributionQueue.add('distribute', { jobId: job.id });
...
2. Müşterinin sorusuna nazikçe, detaylı ve tatmin edici bir şekilde cevap ver. İşte kapsamlı bilgi bankası:
   - **Nasıl Çalışır:** Esnaaf, hizmet alanlar ile hizmet veren profesyonelleri buluşturan ücretsiz bir platformdur. İhtiyacınızı bize söylersiniz, biz bölgenizdeki en uygun hizmet verenleri size eşleştiririz ve onlar size canlı teklifler sunar. Teklifleri karşılaştırıp en uygun olanı seçersiniz.
   - **Ücret / Komisyon:** Hizmet almak, talep oluşturmak ve teklif karşılaştırmak tamamen ÜCRETSİZDİR. Esnaaf, müşterilerden hiçbir ücret veya komisyon almaz. Ödemeyi doğrudan iş bitiminde hizmet vereninize yaparsınız.
   - **Güvenlik:** Platformdaki tüm hizmet verenlerimiz kimlik doğrulaması, oda kaydı ve referans kontrollerinden geçmiş onaylı uzmanlardır. Ayrıca her iş tamamlandıktan sonra 0-10 puan değerlendirmesi yapabilirsiniz. Düşük puan alan hizmet verenler otomatik olarak kalite ekibimiz tarafından incelenir.
   - **Teklif Süreci:** Talebiniz oluşturulduktan sonra bölgenizdeki en uygun 5-10 hizmet veren eşleştirilir. Hizmet verenler genellikle birkaç dakika ile birkaç saat içinde size teklif gönderir. Her hizmet verenin profil puanı, referans sayısı ve teklif fiyatı ekranınızda görüntülenir.
   - **Ödeme:** Ödeme, hizmet tamamlandıktan sonra doğrudan sizinle hizmet veren arasında yapılır (nakit veya istediğiniz yöntemle). Esnaaf bir aracı değildir, buluşturma platformudur.
   - **İptal:** Teklif kabul etmeden önce talebinizi istediğiniz zaman iptal edebilirsiniz. Herhangi bir ceza veya ücret yansımaz.
   - **Hizmet Verene Ulaşma:** Bir hizmet verenin teklifini kabul ettikten sonra, hizmet verenin telefon numarası sizinle paylaşılır. Ayrıca platform içi mesajlaşma ile de iletişim kurabilirsiniz.
   ...
   - **Fatura / Makbuz:** Bu konu doğrudan hizmet veren ile aranızdaki anlaşmaya bağlıdır. Platform üzerinden kayıtlı hizmet verenlerimizin vergi bilgileri kontrol edilmektedir.
...
- Kombi Servisi/Klima Servisi talebi → "Ayrıca doğalgaz tesisatı kontrolü yaptırmak ister misiniz?"
- Fayans Döşeme/Parke Döşeme talebi → "Döşeme sonrası temizlik hizmeti de organize edebiliriz."
- İç Mimar talebi → "Tasarım uygulaması için boya, mobilya montajı veya tadilat ustası da bulabiliriz."
...
Müşterinin talebine göre 'detectCategory' fonksiyonunu çağırırken YALNIZCA aşağıdaki kategorilerden en uygun olanının slug değerini kullanmalısın:
1. 'ev-temizligi' (Ev Temizliği)
2. 'bos-ev-temizligi' (Boş Ev Temizliği)
3. 'boya-badana' (Boya Badana)
4. 'su-tesisati' (Su Tesisatı)
5. 'elektrik-tesisati' (Elektrik Tesisatı)
6. 'ev-tadilat' (Ev Tadilat)
7. 'nakliyat' (Nakliyat / Ev Taşıma)
8. 'hali-yikama' (Halı Yıkama)
9. 'koltuk-yikama' (Koltuk Yıkama)
10. 'insaat-sonrasi-temizlik' (İnşaat / Tadilat Sonrası Temizlik)
11. 'fayans-doseme' (Fayans Döşeme)
12. 'parke-doseme' (Parke Döşeme)
13. 'hasere-ilaclama' (Haşere İlaçlama)
14. 'bocek-ilaclama' (Böcek İlaçlama)
15. 'kombi-servisi' (Kombi Servisi)
16. 'klima-servisi' (Klima Servisi)
17. 'mantolama' (Mantolama)
18. 'dis-cephe' (Dış Cephe)
19. 'marangoz' (Marangoz)
20. 'mobilya-montaji' (Mobilya Montajı)
21. 'ozel-ders' (Özel Ders)
22. 'cam-balkon' (Cam Balkon)
23. 'pvc-pencere' (PVC Pencere)
24. 'ofis-temizligi' (Ofis Temizliği)
25. 'is-yeri-temizligi' (İş Yeri Temizliği)
26. 'dogalgaz-tesisati' (Doğalgaz Tesisatı)
27. 'ic-mimar' (İç Mimar)
28. 'dekorasyon' (Dekorasyon)
29. 'fotografci' (Fotoğrafçı)
30. 'organizasyon' (Organizasyon)
31. 'etkinlik' (Etkinlik)
...
`;
        ...
        if (geminiRes.functionCalls && geminiRes.functionCalls.length > 0) {
          ...
          if (call.name === 'detectCategory') {
            let { categorySlug } = call.args as any;
            if (categorySlug) {
      ...
      if (state.step === 'greeting' || state.step === 'category_detection') {
        ...
        if (nextQ.isComplete) {
          state.step = 'ask_address';
          state.collected_data.hasAskedDetails = true;
          // Capture the last user message as 'details' if the AI just finished on textarea
          if (!state.collected_data.details && state.messages.length > 0) {
             const lastUserMsg = state.messages.filter(m => m.role === 'user').pop();
             if (lastUserMsg) {
                const detailMsg = lastUserMsg.content.trim();
                const isNo = /^(?:hayır|hayir|yok|devam|devam et|istemiyorum|gerek yok|no|skip|geç|gec)$/i.test(detailMsg);
                if (!isNo) state.collected_data.details = detailMsg;
             }
          }
          responseMessage = `Hizmetin verileceği konumu seçebilir misiniz?`;
        } else {
          ...
          if (message.toLowerCase().includes('onayla') || message.toLowerCase().includes('evet') || message.toLowerCase().includes('doğru')) {
            try {
              ...
              if (!category) {
                ...
                if (!seeker) {
                  fallbackResponse = 'Müşteri kaydı bulunamadı. Lütfen giriş yaptığınızdan veya telefon doğrulamasını tamamladığınızdan emin olun.';
                } else {
                    const sendToFavoritesOnly = message.toLowerCase().includes('favori') || message.toLowerCase().includes('favorite');
                    const job = await this.prisma.serviceRequest.create({
                      data: {
  ...
  /**
   * Calculates realistic, service-specific estimated market price range
   */
  public calculateEstimatedPriceRange(slug?: string | null, data: Record<string, any> = {}): { minPrice: number; maxPrice: number; formattedRange: string } | null {
    ...
    // Domain specific base market prices based on 2026 Turkish market research
    let basePrice = 1500;

    if (combined.includes('bilgisayar') || combined.includes('kasa') || combined.includes('laptop')) basePrice = 450;
    else if (combined.includes('utu') || combined.includes('ütü')) basePrice = 400;
    else if (combined.includes('kuru temizleme') || combined.includes('kuru-temizleme')) basePrice = 350;
    else if (combined.includes('sarma') || combined.includes('manti') || combined.includes('mantı')) basePrice = 450;
    else if (combined.includes('hali') || combined.includes('halı')) basePrice = 500;
    else if (combined.includes('yatak')) basePrice = 750;
    else if (combined.includes('cam silme') || combined.includes('cam temizliği') || combined.includes('cam temizligi')) basePrice = 800;
    else if (combined.includes('bocek') || combined.includes('böcek') || combined.includes('ilaclama') || combined.includes('ilaçlama')) basePrice = 850;
    else if (combined.includes('koltuk') && !combined.includes('arac') && !combined.includes('araç')) basePrice = 900;
    else if (combined.includes('yemek')) basePrice = 900;
    else if (combined.includes('petek')) basePrice = 950;
    else if (combined.includes('buharli') || combined.includes('buharlı')) basePrice = 1100;
    else if (combined.includes('arac') || combined.includes('araç') || combined.includes('araba')) basePrice = 1200;
    else if (combined.includes('apartman') || combined.includes('merdiven')) basePrice = 1200;
    else if (combined.includes('ev-temizligi') || combined.includes('ev temizliği')) basePrice = 1500;
    else if (combined.includes('ofis') || combined.includes('dukkan') || combined.includes('dükkan')) basePrice = 1800;
    else if (combined.includes('bos-ev') || combined.includes('boş ev')) basePrice = 2400;
    else if (combined.includes('su-deposu') || combined.includes('su deposu')) basePrice = 2600;
    else if (combined.includes('insaat') || combined.includes('inşaat') || combined.includes('tadilat-sonrasi')) basePrice = 2800;
    else if (combined.includes('mermer') || combined.includes('cila')) basePrice = 3200;
    else if (combined.includes('dis-cephe') || combined.includes('dış cephe')) basePrice = 3500;
    // Tadilat & Tesisat & Nakliyat
    else if (combined.includes('boya') || combined.includes('badana')) basePrice = 4500;
    else if (combined.includes('parke')) basePrice = 5000;
    else if (combined.includes('fayans') || combined.includes('seramik')) basePrice = 6000;
    else if (combined.includes('nakliyat') || combined.includes('nakliye')) basePrice = 6500;
    else if (combined.includes('tadilat')) basePrice = 25000;
    else if (combined.includes('su-tesisat') || combined.includes('tesisat')) basePrice = 850;
    else if (combined.includes('kombi')) basePrice = 750;
    else if (combined.includes('klima')) basePrice = 1100;

    let multiplier = 1.0;
    const dataStr = JSON.stringify(data).toLowerCase();
    if (dataStr.includes('2+1') || dataStr.includes('2_1') || dataStr.includes('50 - 150') || dataStr.includes('5-8') || dataStr.includes('4-7') || dataStr.includes('laptop')) multiplier = 1.2;
    else if (dataStr.includes('3+1') || dataStr.includes('3_1') || dataStr.includes('150 - 300') || dataStr.includes('9-15') || dataStr.includes('8-12') || dataStr.includes('gaming')) multiplier = 1.45;
    else if (dataStr.includes('4+1') || dataStr.includes('4_1')) multiplier = 1.75;
    else if (dataStr.includes('5+1') || dataStr.includes('6+1') || dataStr.includes('villa') || dataStr.includes('300 m²') || dataStr.includes('15_plus') || dataStr.includes('12_plus') || dataStr.includes('server')) multiplier = 2.3;
    ...
    // Min ~ 0.67x center, Max ~ 1.67x center
  ...
  private async detectCategory(message: string): Promise<{ detected: boolean; categorySlug: string | null; categoryName: string | null; confidence: number }> {
    ...
    if (text.includes('pvc') || text.includes('panjur')) {
      return { detected: true, categorySlug: 'pvc-pencere', categoryName: 'PVC Pencere', confidence: 0.95 };
    }
    if (text.includes('ofis temizliği') || text.includes('ofis temizligi')) {
      return { detected: true, categorySlug: 'ofis-temizligi', categoryName: 'Ofis Temizliği', confidence: 0.95 };
    }
    if (text.includes('iş yeri temizliği')) {
      return { detected: true, categorySlug: 'is-yeri-temizligi', categoryName: 'İş Yeri Temizliği', confidence: 0.95 };
    }
    ...
    if (text.includes('iç mimar') || text.includes('tasarım') || text.includes('ic mimar')) {
      return { detected: true, categorySlug: 'ic-mimar', categoryName: 'İç Mimar', confidence: 0.95 };
    }
    if (text.includes('dekorasyon')) {
      return { detected: true, categorySlug: 'dekorasyon', categoryName: 'Dekorasyon', confidence: 0.95 };
    }
    ...
    if (text.includes('organizasyon')) {
      return { detected: true, categorySlug: 'organizasyon', categoryName: 'Organizasyon', confidence: 0.95 };
    }
    ...
    if (text.includes('fayans') || text.includes('seramik')) {
      return { detected: true, categorySlug: 'fayans-doseme', categoryName: 'Fayans Döşeme', confidence: 0.95 };
    }
    if (text.includes('parke') || text.includes('döşeme')) {
      return { detected: true, categorySlug: 'parke-doseme', categoryName: 'Parke Döşeme', confidence: 0.95 };
    }
    if (text.includes('haşere') || text.includes('ilaçlama')) {
      return { detected: true, categorySlug: 'hasere-ilaclama', categoryName: 'Haşere İlaçlama', confidence: 0.95 };
    }
    ...
    if (text.includes('klima')) {
      return { detected: true, categorySlug: 'klima-servisi', categoryName: 'Klima Servisi', confidence: 0.95 };
    }
    if (text.includes('mantolama') || text.includes('yalıtım')) {
      return { detected: true, categorySlug: 'mantolama', categoryName: 'Mantolama', confidence: 0.95 };
    }
    if (text.includes('dış cephe')) {
      return { detected: true, categorySlug: 'dis-cephe', categoryName: 'Dış Cephe', confidence: 0.95 };
    }
    if (text.includes('marangoz')) {
      return { detected: true, categorySlug: 'marangoz', categoryName: 'Marangoz', confidence: 0.95 };
    }
    if (text.includes('mobilya montaj') || text.includes('kurulum') || text.includes('gardırop')) {
      return { detected: true, categorySlug: 'mobilya-montaji', categoryName: 'Mobilya Montajı', confidence: 0.95 };
    }
    ...
    if (text.includes('boş ev temizliği') || text.includes('bos ev temizligi') || text.includes('boş ev')) {
      return { detected: true, categorySlug: 'bos-ev-temizligi', categoryName: 'Boş Ev Temizliği', confidence: 0.95 };
    }
    ...
    try {
      ...
      const allRoutes: any[] = []; // await this.prisma.graphCategoryRoute.findMany();
  ...
  private async initializeCategoryFlow(state: SessionState, slug: string, name?: string) {
    ...
    if (route) {
      ...
      state.collected_data.is_graph_flow = false; // Flag for graph routing (Disabled)
    } else {
  ...
  private getQuestionsForCategory(slug: string, state?: SessionState): any[] {
    ...
    const districtQuestions: Record<string, string> = {
      ...
      'bos-ev-temizligi': 'Hizmetin verileceği ilçeyi (örn. Kadıköy, Şişli) yazar mısınız?',
...
Kullanıcının cevabı hangi geçerli seçeneğe karşılık geliyor? SADECE seçeneğin value (değer) kısmını yaz. Eğer hiçbirine uymuyorsa "INVALID" yaz.${isMulti ? ' Birden fazla seçenek eşleşiyorsa aralarına virgül koyarak yaz.' : ''}`;
  ...
  private getChecklistForCategory(slug: string | null): string[] {
    switch (slug) {
      ...
      case 'bos-ev-temizligi':
        return [
          'Ev kaç odalı ve kaç banyolu?',
          'Evin boş olma sebebi nedir (kiracı çıktı, sıfır ev vb.)?'
        ];
      case 'boya-badana':
        return [
