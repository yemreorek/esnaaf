...
- **KVKK Onay Kutusu (Talep Formu):** Müşteri uygulaması sohbet akışının son adımındaki Talep Onay Kartı'na KVKK Aydınlatma Metni onay kutusu (`kvkkConsent` checkbox) eklendi. Müşterinin KVKK metnini onaylamadan "Onayla" demesi durumunda tarayıcı uyarısı verilerek işlem engellenmektedir. Hem buton tıklaması hem de el yazısıyla "Onayla/Evet" yazılması durumunda bu kontrol devreye sokularak KVKK uyumluluğu uçtan uca sağlandı.
