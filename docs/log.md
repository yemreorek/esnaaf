...
- **Canlı Sohbet Akan Yazı Efekti (Streaming/Typing Effect):** Müşteri uygulamasındaki canlı sohbet penceresinde, AI asistanının (Gemini) yanıtlarının tek seferde aniden belirmesi yerine, Gemini ve ChatGPT arayüzlerinde olduğu gibi karakter karakter akarak gelmesi (typing effect) sağlandı. Yazma animasyonu süresince kullanıcının çift mesaj gönderimi engellendi ve konuşma akışındaki özet kartları ile yönlendirme adımları ancak yazı tamamen bittikten sonra görünür kılınarak premium bir sohbet deneyimi (UX) sunuldu. Titremeleri önlemek amacıyla auto-scroll mekanizması akış esnasında anlık (`behavior: 'auto'`) kaydırma yapacak şekilde güncellendi.
